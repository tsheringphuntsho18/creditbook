...
const CustomerDashboard: React.FC<CustomerDashboardProps> = ({
  ...
  const shopsData = useMemo(() => {
    const customerTxs = transactions.filter(
      (tx) => tx.customerId === customer.phoneNumber
    );
    ...
    return Object.keys(txsByVendor)
      .map((vendorId) => {
        const vendor = vendors.find((v) => v.phoneNumber === vendorId);
        if (!vendor) return null;

        const vendorTxs = txsByVendor[vendorId].sort(
          (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
        );
        const balance = vendorTxs.reduce(
          (acc, t) =>
  ...
  const selectedShopData = selectedVendorId
    ? shopsData.find((d) => d.vendor.phoneNumber === selectedVendorId)
    : null;
  ...
  if (selectedShopData) {
    ...
    const totalPages = Math.ceil(
      shopTransactions.length / TRANSACTIONS_PER_PAGE
    );
    const paginatedTransactions = shopTransactions.slice(
            ...
            <div className="flex justify-between items-center py-4">
              <div className="flex items-center gap-4">
                <button
                  ...
                  className="text-content-light-secondary dark:text-content-dark-secondary hover:text-content-light-primary dark:hover:text-content-dark-primary"
                >
                  <ArrowLeftIcon />
                </button>
                {vendor.shopLogo ? (
                  ...
                  <div className="flex items-center justify-center h-12 w-12 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-400">
                    <ShopIcon />
                  </div>
                )}
                <div>
              ...
              <div className="text-right">
                <p className="text-sm font-medium text-content-light-secondary dark:text-content-dark-secondary">
                  Balance
                </p>
            ...
            <div className="flex items-center gap-2">
              <div
                className={`w-3 h-3 rounded-full ${
                  isShopOpen ? "bg-success animate-pulse" : "bg-danger"
                }`}
              ></div>
              <span
                className={`font-bold ${
                  isShopOpen ? "text-success" : "text-danger"
                }`}
              >
          ...
          <div className="bg-background-light-primary dark:bg-background-dark-secondary p-6 rounded-xl shadow-lg space-y-4">
            ...
            <div className="space-y-3">
              {paginatedTransactions.length === 0 ? (
                <p className="text-center text-content-light-secondary dark:text-content-dark-secondary py-10">
                  No transactions with this shop.
                </p>
              ) : (
                paginatedTransactions.map((t) => (
                  <div
                    key={t.id}
                    className="p-4 bg-background-light-secondary dark:bg-background-dark-primary border-b border-gray-200 dark:border-gray-700"
                  >
    ...
    <div className="w-full min-h-screen bg-background-light-secondary dark:bg-background-dark-primary">
      <header className="bg-background-light-primary dark:bg-background-dark-secondary shadow-md sticky top-0 z-40">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div>
              <h1 className="text-xl font-bold text-content-light-primary dark:text-content-dark-primary">
                Welcome, {customer.name}!
              </h1>
              <p className="text-sm text-content-light-secondary dark:text-content-dark-secondary">
                Select a shop to view your ledger.
              </p>
            </div>
            <div className="flex items-center gap-4">
              <NotificationBell
              ...
              <button
                onClick={onLogout}
                className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-danger rounded-lg hover:bg-danger-light focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-danger"
              >
                <LogoutIcon />
        ...
        {shopsData.length === 0 ? (
          <div className="text-center p-10 bg-background-light-primary dark:bg-background-dark-secondary rounded-lg shadow-md">
            <p className="text-content-light-secondary dark:text-content-dark-secondary">
              You don't have a credit history with any shop yet.
            </p>
          </div>
        ) : (
          shopsData.map(({ vendor, balance }) => {
            ...
            return (
              <button
                ...
                onClick={() => setSelectedVendorId(vendor.phoneNumber)}
                className="w-full text-left p-4 bg-background-light-primary dark:bg-background-dark-secondary rounded-lg shadow-md hover:shadow-lg hover:border-primary border border-transparent transition-all flex items-center justify-between"
              >
                <div className="flex items-center gap-4">
                  {vendor.shopLogo ? (
                    <img
                  ...
                  <div className="flex-grow">
                    ...
                    {vendor.shopDescription && (
                      <p
                        className="text-xs italic text-content-light-secondary dark:text-content-dark-secondary mt-1 break-words max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl 2xl:max-w-2xl whitespace-pre-line"
